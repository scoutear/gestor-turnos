...
const API_URL = "https://script.google.com/macros/s/AKfycbxgV7aRpuo0al3UmcQuDz7CZ_KOM3m2z70klW8efVLQNYz2xQtjaYzm7ng-dZDl13uA/exec";
...
export default function App(){
    ...
    fetch(`${API_URL}?action=reservas`)
      ...
      .then(rows=>{
        ...
        rows.forEach(r=>{
          ...
          const slots = SLOTS.slice(SLOTS.indexOf(startSlot),SLOTS.indexOf(startSlot)+4);
          slots.forEach(s=>{
  ...
  const reserve = ()=>{
    ...
    }).then(()=>{ setSelected(null); cargarReservas(); });
  };

  /* ======= LIBERAR ======= */
  const release = ()=>{
    alert("Para liberar: borrar fila en Sheet (etapa siguiente)");
    setSelected(null);
  };
      ...
      <div className="card table">
        <table>
          <thead>
            <tr>
              <th>Hora</th>
              {days.map((d,i)=><th key={i}>{DAYS[i]} {d.getDate()}</th>)}
            </tr>
          </thead>
          <tbody>
            {SLOTS.map(slot=>(
              <tr key={slot}>
                <td><b>{minutesToTime(slot)}</b></td>
                {days.map((_,di)=>{
                  const r = reservas[`${weekKey}_${di}_${slot}`];
                  return (
                    <td key={di}>
                      <div className={`cell ${r?"reserved":"free"}`} onClick={()=>toggleCell(di,slot)}>
                        {r ? r.name : "LIBRE"}
                      </div>
                    </td>
                  );
